...
/**
 * 单例模式
 * 某些场景要求一个对象只能有一个实例，可以通过单例模式实现。由于只能有一个
 * 实例，因此该类的构造器必须用private修饰来禁止通过new操作符来新建实例。
 * 类内持有一个private修饰的本类类型的实例，又因为只能有一个实例，所以使其
 * 为类成员，如该类类名为Singleton，可声明为：
 * private static Singleton instance;
 * 通过类内的一个public修饰的getIntance方法来获取实例，该方法也是static的。
 * 根据实例产生的时机分为饿汉模式和懒汉模式，懒汉模式中有线程安全和不安全的写法。
 * 
 * 饿汉模式
 * 类属性Singleton instance在声明时就实例化。
 * private static Singleton instance = new Singleton();
 * 懒汉模式
 * 类属性Singleton只声明但不实例化，将实例化动作放到getInstance方法中，则
 * 只有调用getInstance时才能获取Singleton的实例。
 * 懒汉模式因为在调用getInstance时才获得实例，存在多线程竞争的问题，因此可以
 * 结合volatile(避免指令重排)和synchronized(保证原子性)以双锁检测方式将
 * Singleton写成线程安全的类。
 * 
