...
/**
 * 工厂方法模式
 * 在简单工厂模式中，当出现产品类的增删改时，均需要修改静态产品工厂类内的
 * 选择分支，破坏了开闭原则。针对这个问题，可以将生产不同产品的工厂也单独
 * 成类，他们继承同一个抽象工厂类。这样在出现新产品时只增加相应的产品类和
 * 生产他的工厂类，对其他代码均无需改动其他。等到要使用这个新工厂时，再在
 * 客户端中声明这个工厂的实例，由它来生产产品即可。这就是符合开闭原则的
 * 工厂方法模式。
 * 
 * 本示例以四则运算为例，演示客户端如何声明一个具体的运算工厂并通过该工厂
 * 生产相应的运算产品，然后执行该产品的产品方法(执行运算)返回结果。
 * 
 * 结构
 * 抽象工厂类
 *   定义一个创建产品的抽象方法。
