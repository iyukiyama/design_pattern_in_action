...
/**
 * 享元模式
 * 某些对象相似度很高，只有少量字段或方法不同，当需要大量这类对象时，若为每一
 * 次需求都创建实例，内存开销会很大。此时可以提炼出这些对象的相同部分，作为
 * 所谓的享元，创建一个享元工厂类，持有享元实例。需要使用上述对象时，从享元
 * 工厂中获取该实例。若这些对象有不同部分，提取这些不同部分单独成类，使用时
 * 作为享元中的方法参数传入享元(使相同和不同部分结合)。这样无论使用多少次，
 * 对于享元而言，只有一个实例的开销。
 * 
 * 本示例以网站为抽象享元，博客网站和电子商务网站为具体享元，用户为外部非享元。
 * 展示如何通过享元工厂类添加和获取享元，如何将非享元作为享元方法的参数传入到
 * 享元内。
 * 
 * 结构
 * 抽象享元类
