...
/**
 * 适配器模式
 * 当前有一工具(一个已存在的类)，能实现若干功能(方法)，需要在某平台(目标)
 * 上使用，但与平台的接口不同(已存在的类的方法与接口方法名不同)，需提供
 * 一个适配器(适配器类)转换为平台接口。这种场景可以使用适配器模式实现。
 * 与代理模式比较类似，但代理类中的RealSubject和Proxy都需要实现Subject
 * 抽象类。而适配器模式中的Adaptee类是原本就已存在的类，内部的方法与接口
 * 中定义的抽象方法不同(行为一致，方法名不同)。
 * 
 * 本示例中，以TypeC为抽象目标类，其内有标准充电方法typecChange和标准
 * 数据传输方法typecDataTrans。现有Typeb类和Lightning类，均有自己的
 * 充电方法和数据传输方法。演示如何为他们设置适配器类，使得客户端能够通过
 * 调用适配器中的符合TypeC的标准方法来间接使用Typeb和Lightning的充电和
 * 数据传输功能(方法)。
 * 
 * 结构
 * 抽象目标类
