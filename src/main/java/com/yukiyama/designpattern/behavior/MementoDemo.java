...
/**
 * 备忘录模式
 * 以游戏中的存档和恢复存档为例，在游戏过程中需要保存当前游戏状态(包括各种
 * 游戏角色属性)作为存档，在之后的某个时刻能够恢复这个存档。文档中的撤销
 * 操作也有类似的需求。此类场景可以用备忘录模式(也叫快照模式)实现。该模式
 * 涉及三个角色，发起人类，备忘录类和备忘录管理者类。发起人类管理系统当前
 * 状态，并有一个保存为备忘的存档方法，一个恢复为指定备忘版本的恢复存档方法。
 * 备忘录类只管理该备忘的状态信息，而备忘录管理者只管理备忘录实例。符合单一
 * 职责原则。
 * 
 * 本示例客户端中展示备忘录使用过程。声明一个发起者，该发起者设置系统状态后
 * 存档。然后创建当前系统存档(备忘录)，并让一个备忘录管理者持有。接着
 * 该发起者修改系统状态，最后调用存档恢复方法恢复到修改前状态。
 * 
 * 结构
 * 发起人类
