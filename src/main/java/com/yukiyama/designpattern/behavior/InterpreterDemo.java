...
/**
 * 解释器模式
 * 解释器用来处理这样一类对象，该对象可不断划分为更小的对象，使得对该对象
 * 的处理和对再分之后的部分的处理类似，且对于不断再分后得到到的最小单位，有
 * 最终处理。例如输入歌曲解析出简谱，每个曲子可以划分为更短的曲子，但解析是
 * 类似的，直至单个音符可以直接输出简谱符号(或按定义好的规则转换)。该模式
 * 类似组合模式，但解释器模式处理对象的元素通常比组合模式的要多，且组合模式
 * 是对象结构模式，而解释器 模式是类行为模式。在使用上，组合模式在客户端以树
 * 的结构将所有节点按层次组合为一个整体再处理。而对与解释器模式，在客户端
 * 直接输入整体，处理时再循环或迭代解释(处理) 非终结表达式直到将迭代至终结符
 * 表达式，且所有终结符表达式都被解释完毕。
 * 
 * 终结符与非终结符的简单理解：
 * 非终结符：可再分为非终结符和终结符的元素，例如“我吃饭”可再分为“我(代词，
 * 主语)”,“吃饭(动词，谓语)”,“饭(名词，宾语)”。
 * 终结符：不可再分的语法元素，例如上述的“我”，“吃”，“饭”。
 * 参考：
 * https://en.wikipedia.org/wiki/Terminal_and_nonterminal_symbols
 * 
 * 本示例定义一种关于加法运算的规则，只有符号"a","b","A","B"之间的加减法
 * 是合法运算。客户端将输入一些表达式，通过解释器来判断(解释)该表达式是否符合
 * 语法定义，合法输出true，不合法输出false。客户端声明一个上下文类Context，
 * 将要解释的表达式传入Context的interpret方法中，返回解释结果。Context类
 * 内持有一个非终结符表达式实例(以抽象表达式类声明)NonterminalExpression，
 * 并通过无参构造器初始化该实例。NonterminalExpression的构造器是有参的，
 * 参数是终结符表达式实例TerminalExpression。
 * 
 * 结构
 * 抽象表达式类
 *   定义一个抽象解释方法interpret(String expression)。
 * 终结符表达式类
 *   继承抽象表达式类。持有一个Set<String>泛型的终结符集合。实现抽象表达式类的
 *   抽象解释方法interpret。
 * 非终结符表达式类
 *   继承抽象表达式。持有所有种类的终结符表达式实例(以AbstractExpress类型)。
 *   实现抽象表达式类中的抽象解释方法interpret。
 * 环境类(上下文类)
 *   持有所有种类的终结符集合(以String数组类型)，持有一个非终结符实例
 *   (以AbstractExpression类型)。
