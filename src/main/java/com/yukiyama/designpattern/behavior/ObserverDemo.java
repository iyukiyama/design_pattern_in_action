...
/**
 * 观察者模式
 * 当一个对象变化时，所有与他有关联关系的对象都将因为这个变化而变化，将
 * 前者称为目标对象，后者称为观察者对象。例如以人行道上的红绿灯为目标对象，
 * 行人和车辆为观察者对象。当红绿灯从红灯变为绿灯时，行人因为这个变化而
 * 从等待转变为过马路，而汽车则从通过转变为等待，这些观察者都对目标的变化
 * 做出了反映。此类场景可以用观察者模式实现。
 * 
 * 本示例以人民币汇率为目标对象，进口公司和出口公司作为观察者对象。演示目标
 * 对象变化时，如何通知所有观察者做出其响应的动作。
 * 
 * 结构
 * 抽象目标类
 *   观察者需要观察的对象，类内实现添加和删除观察者的普通方法，定义一个通知
 *   所有观察者的抽象方法。
 * 具体目标类
 *   继承抽象目标类，实现抽象方法。
 * 抽象观察者类
 *   持有一个name属性，并有对应的getter/setter。定义了一个响应目标对象
 *   变化的抽象方法action，被目标类通知时执行。
 * 具体观察者类
