...
/**
 * 责任链模式
 * 工作中可能有这样的请求处理场景，例如假期审批，会根据申请假期的天数长短，
 * 由不同权限的主管者审批，但提交申请时总是提交给自己的直接主管，由直接主管
 * 来判断是否需要上升到更高权限的管理者来审批。类似场景可以描述为一个请求
 * 根据其内容由不同级别的处理者处理，申请入口总是最低级别的处理者。在客户
 * 端中处理此类请求，最简单直接的做法用if-else语句遍询所有处理者，最终总能
 * 够被某一权限的处理者处理。这种做法的缺点是客户端责任太大，违背了单一职责
 * 原则，另外可扩展性也很差，当需要修改或删减处理者时，需要修改客户端代码。
 * 对于这种场景可以使用责任链模式实现，创建每一层级的处理者类，内部均实现一个
 * 处理方法，请求从最底层处理者开始，每一层级处理者处理请求时，如判断不在权限
 * 范围内，则向后继处理者传递该请求，直至最后一层。这种做法很好地实践了单一
 * 职责原则，由于每一层级处理者均为一类，对处理者的变化也有良好的扩展能力。
 * 
 * 本示例以假期申请为例，演示不同天数的申请如何通过责任链传递从最低层级管理者
 * 传递到具有相应权限的管理者处并得到处理。
 *
 * 结构
 * 抽象处理者类：
 *   处理请求的对象，类内实现一个设置后继处理者的普通方法，定义一个处理
 *   请求的抽象方法。
 * 具体处理者类
 *   继承抽象处理者类，实现抽象方法。
 * 请求类(可选)
 *   当请求比较复杂时，可以将其封装成类。当请求比较简单，如申请假期的场景，
 *   可以只用int表示请求。
 */
