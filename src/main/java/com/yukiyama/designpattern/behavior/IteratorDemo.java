...
/**
 * 迭代器模式
 * 对于由多个相同元素聚合而成的对象，需要遍历内部元素时，最直接的做法是在客户端
 * 通过一个for循环来执行遍历操作。该做法的缺点是对客户端暴露了聚合对象的内部，
 * 且增加了客户端的负担。另一种做法是在聚合对象内封装一个遍历方法，在客户端调用
 * 该方法。缺点同样明显，当要改变遍历方式(例如原本是从前往后，改成从后往前)，
 * 就需要修改聚合类中的遍历方法，违背开闭原则。针对这种场景，结合前两种方式的特点，
 * 可以将遍历方法抽离分离出来，但不是在客户端中实现，而是创建一个迭代器类，类中
 * 持有聚合对象，并实现迭代方法。这样对客户端不用暴露聚合对象内部，又实现了遍历
 * 方法和聚合对象的分离。
 * 
 * 本示例展示如何在客户端中通过自定义迭代器遍历自定义聚合类内的元素。客户端声明
 * 一个具体聚合类(以抽象聚合接口类型声明)，然后初始化该聚合的元素。再通过聚合类
 * 的getIterator方法获取具体迭代器类实例(以抽象迭代器类型)。最后调用该Iterator
 * 的相关方法完成遍历。
 * 
 * 结构
 * 迭代器接口
 ...
 * 具体迭代器类
 *   迭代器接口的实现类。持有聚合元素的集合List<Object>，持有当前处理
 *   元素的下表index。实现fist，next和hasNext三个接口方法。
 * 抽象聚合接口
 *   定义聚合角色的方法，增加聚合元素的方法add，移除聚合元素的方法remove，
 *   获取一个迭代器示例的方法getIterator。
 * 具体聚合类
 *   抽象聚合接口的实现类，以List<Object>持有一个聚合示例。实现抽象聚合接口
 *   中的add，remove，getIterator方法。
 */
